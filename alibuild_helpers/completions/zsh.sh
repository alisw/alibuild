...
_aliBuild_cmd_build() {
  _arguments -s -S \
    ...
    '--plan[Resolve the build and print what it would do, then stop]' \
    ...
    '--legacy-links-store[Where to publish the legacy TARS/ links and store objects]:store: ' \
    '--cas-public-url[Public base URL of the CAS bucket, for legacy store redirects]:url: ' \
    '--storage[Retention for uploaded tarball blobs]:storage:(ephemeral permanent)' \
    '--no-snapshot-sources[Do not archive built package sources into the reapi ledger]' \
    '--sign-url[Security-proxy sign route for reapi Action Cache uploads]:url: ' \
    '--sign-token[Gate token presented to --sign-url]:token: ' \
    '--sign-token-file[File holding the credential for --sign-url, re-read per request]:file:_files' \
    '--signer[Human-readable signer label recorded in the signature]:name: ' \
    '--no-sign[Upload to a reapi:// store without signing Action Cache entries]' \
    '--require-signature[Verify signatures on reused reapi tarballs]:policy:(off warn require)' \
    '--trusted-keys[JSON keyring of trusted signing keys]:keyring:_files' \
