...
_aliBuild_complete() {
  ...
  case "$subcmd" in
    build)
      case "$prev" in
        -a|--architecture|-z|--devel-prefix|-e|-j|--jobs|--plugin|--docker-image|--docker-extra-args|-v|--remote-store|--write-store|--ac-store|--legacy-links-store|--cas-public-url|--sign-url|--sign-token|--signer)
          return ;;
        --storage)
          COMPREPLY=( $(compgen -W "ephemeral permanent" -- "$cur") ); return ;;
        --require-signature)
          COMPREPLY=( $(compgen -W "off warn require" -- "$cur") ); return ;;
        --sign-token-file|--trusted-keys)
          _filedir; return ;;
        --defaults)
      ...
      if [[ "$cur" == -* ]]; then
        COMPREPLY=( $(compgen -W "
          ...
          --annotate --only-deps --plan
          --docker --docker-image --docker-extra-args -v
          --no-remote-store --remote-store --write-store --ac-store --storage --insecure
          --legacy-links-store --cas-public-url
          --no-snapshot-sources --sign-url --sign-token --sign-token-file --signer --no-sign
          --require-signature --trusted-keys
          -C --chdir -w --work-dir -c --config-dir --reference-sources
