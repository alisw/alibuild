...
def asList(x): return x if type(x) == list else [x]
...
def getPackageList(packages, specs, configDir, preferSystem, noSystem,
    ...
    while packages:
        ...
        def fn(what): return disabledByArchitectureDefaults(architecture, defaults, spec.get(what, []))
        spec["disabled"] += [x for x in fn("requires")]
        spec["disabled"] += [x for x in fn("build_requires")]
        def fn(what): return filterByArchitectureDefaults(architecture, defaults, spec.get(what, []))
