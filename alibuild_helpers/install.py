...
def add_parser(subparsers, detected_arch, work_dir_default):
  """Register the `install` subcommand's parser -- args live with the command, so
  the shared top-level argument parser carries no install-specific reapi options."""
  ...
  p.add_argument("package", metavar="PACKAGE", help="Package to install.")
  ...
  p.add_argument("--revision", default=None, metavar="REVISION",
                 help="Revision to install. Defaults to the highest available for the version.")
  add_reapi_store_args(
    p, remote_help="reapi:// store to install from.",
    arch_help="Architecture to install for. Default '%(default)s'.",
    detected_arch=detected_arch, work_dir_default=work_dir_default,
    work_dir_help="Default install prefix if --prefix is not given. Default '%(default)s'.")
  p.add_argument("--prefix", dest="prefix", default=None, metavar="DIR",
...
def install_entry(sync, entry, prefix, architecture, checker=None):
  ...
  When a SignatureChecker is passed, the downloaded blob is bound to the signed
  output digest (bytes must hash to it) before it is unpacked into the prefix."""
  action = entry["action"]
  ...
  if os.path.isdir(dest_dir):
    ...
    try:
      ...
      if checker:
        checker.check_blob(tmp_tarball, algo, content_hash, entry)
...
def doInstall(args, parser):
  ...
  for entry in closure:
    install_entry(sync, entry, prefix, args.architecture, checker)
