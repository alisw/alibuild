...
from alibuild_helpers.cmd import execute, DockerRunner, AppleContainerRunner
    ...
    def test_AppleContainerRunner(self, mock_getstatusoutput, mock_getoutput):
        ...
        with AppleContainerRunner("image", ["extra arg"], extra_env={}, extra_volumes=[]) as getstatusoutput_container:
            mock_getoutput.assert_called_with(["container", "run", "--detach", "--rm", "--entrypoint=/bin/sleep",
                                               "extra arg", "image", "inf"])
            getstatusoutput_container("echo foo")
            mock_getstatusoutput.assert_called_with(["container", "exec", "container-id", "bash", "-c", "echo foo"], cwd=None)
        mock_getstatusoutput.assert_called_with("container kill container-id")

        mock_getoutput.reset_mock()
        mock_getstatusoutput.reset_mock()
        with AppleContainerRunner("", [], extra_env={}, extra_volumes=[]) as getstatusoutput_container:
            mock_getoutput.assert_not_called()
            getstatusoutput_container("echo foo")
            mock_getstatusoutput.assert_called_with("/bin/bash -c 'echo foo'", cwd=None)
            mock_getstatusoutput.reset_mock()
