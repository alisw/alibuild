...
class TestRecipes(unittest.TestCase):
  ...
  def test_validateDefault(self):
    ...
    ok, out, validDefaults = validateDefaults({"package": "foo","valid_defaults": ["o2", "o2-daq"]}, "release")
    self.assertEqual(ok, False)
    self.assertEqual(out, 'Cannot compile foo with `release\' default. Valid defaults are\n - o2\n - o2-daq')
    ok, out, validDefaults = validateDefaults({"package": "foo","valid_defaults": ["o2", "o2-daq"]}, "o2")
    self.assertEqual(ok, True)
    ok, out, validDefaults = validateDefaults({"package": "foo","valid_defaults": "o2-daq"}, "o2")
    self.assertEqual(ok, False)
    self.assertEqual(validDefaults, ["o2-daq"])
